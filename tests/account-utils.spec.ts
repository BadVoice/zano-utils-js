import { accountValidate, generateAccount } from '../src';
...
describe('address validation', () => {
  ...
  it('created address should be valid', async () => {
    await expect(accountValidate(address, publicSpendKey, publicViewKey, secretSpendKey, secretViewKey))
      .resolves
      .toBe(true);
  });

  it('created address non valid', async () => {
    await expect(accountValidate('', '', '', '', ''))
      .rejects
      .toThrow('invalid address format');
  });

  it('address on invalid privateKey', async () => {
    await expect(accountValidate(address, '', '', '', ''))
      .rejects
      .toThrow('invalid address keys');
  });

  it('address on invalid publicKey', async () => {
    await expect(accountValidate(address, publicSpendKey, publicViewKey, secretSpendKey2, secretViewKey))
      .rejects
      .toThrow('invalid depend secret view key');
  });

  it('address on invalid publicSpendKey', async () => {
    await expect(accountValidate(address, publicSpendKey2, publicViewKey, secretSpendKey, secretViewKey))
      .rejects
      .toThrow('invalid address keys');
  });

  it('address on invalid secret view key', async () => {
    await expect(accountValidate(address, publicSpendKey2, publicViewKey, secretSpendKey, ''))
      .rejects
      .toThrow('invalid address keys');
  });

  it('address on invalid depend secret view key', async () => {
    await expect(accountValidate(address, publicSpendKey, publicViewKey, secretSpendKey, ''))
      .rejects
      .toThrow('invalid depend secret view key');
  });

  it('address on invalid publicViewKey', async () => {
    await expect(accountValidate(address, publicSpendKey, publicViewKey2, secretSpendKey, secretViewKey))
      .rejects
